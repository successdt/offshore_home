...
function cmsmasters_featured_block($atts, $content = null) {
    ...
    $out = '<div class="featured_block">' . 
		'<div class="colored_title">' . 
			'<div class="colored_title_inner glow_' . $bgcolor . '">' . 
	...
	if ($button != '') {
		...
		$out .= '><span class="icon_banner"></span><span>' . $buttontext . '</span>' . 
			'</a>';
    }
...
function cmsmasters_colored_block($atts, $content = null) {
    ...
    $out = '<div class="colored_banner glow_' . $bgcolor . '">' . do_shortcode($content);
...
function cmsmasters_person_block($atts, $content = null) {
	...
	$out = '<div class="cmsms_our_team_wrap"><div class="cmsms_our_team">';
	...
	$out .= '<div class="cmsms_team_rollover glow_' . $color . '">';
	
	$out .= '<header class="entry-header"><div>' . 
		'<h6 class="person_title">' . $title . '</h6>';
	
	
	if ($subtitle != '') {
		$out .= '<p class="person_title">' . $subtitle . '</p>';
	}
...
function cmsmasters_pricing_table($atts, $content = null) {
	...
	$out = '<div class="cmsms_pricing_table">' . 
		...
		'<div class="pricing_footer">' . 
			'<a class="pricing_button buy glow_' . $color . '" href="' . $buttonlink . '">' . $buttontext . '</a>' . 
		'</div>' . 
	'</div>';
	...
	return '<div class="percent_item">' . 
		...
		'<div class="percent_item_colored_main_wrap">' . 
			'<div class="percent_item_colored_wrap" style="width:' . $value . '%;">' . 
				'<div class="percent_item_colored" style="background-color:' . $color . ';"><span class="percent_item_text">' . $value . '%</span></div>' . 
			'</div>' . 
		'</div>' . 
	'</div>';
}
...
function posttype_shortcode($atts, $content = null) {
	...
	if ($post_slide == 'true') {
		...
		$out = '<section class="post_type_shortcode"><div class="post_type_shortcode_inner">';
    ...
    if ($posttype_query->have_posts()) :
        while ($posttype_query->have_posts()) : $posttype_query->the_post();
			...
			if ($post_type != 'testimonial') {
				...
				$out .= '<div class="project_rollover"><span class="cmsms_post_format_img"></span>' .
					...
					if ($show_content == 'true' && theme_excerpt(20, false)) {
				...
				if ($show_images == 'true') {
					if (has_post_thumbnail() != '') {
						...
						$out .='<img src="' . get_template_directory_uri() . '/img/testimonials_avatar.jpg' . '" style="width:40px; height:40px;" />';
					}
				}
				if ($show_info == 'true') {
