...
function optionsframework_fields() {
	...
	foreach ( $options as $value ) {
		...
		case 'typography':
			...
			// Font Face - system
			$output .= '<optgroup label="System"></optgroup>';	
			...
			$output .= '<optgroup label="Google Fonts"></optgroup>';
			$google_fonts = doover_google_fonts();			
			foreach ( $google_fonts as $gfont ) {
				$glabel = str_replace("+", " ", $gfont);
				$output .= '<option value="' . esc_attr( $gfont ) . '" ' . selected( $typography_stored['face'], $gfont, false ) . '>' . esc_html( $glabel ) . '</option>';
			}
		...
		case 'prefix_text':
			$prefix_text = $val;
			$output .= '<input id="' . esc_attr( $value['id'] .'_prefix' ) . '" class="of-input of-tiny mr-5" name="' . esc_attr( $option_name . '[' . $value['id']. '][prefix]') . '" type="text" value="' . esc_attr( $prefix_text['prefix'] ) . '" />';
			$output .= '<input id="' . esc_attr( $value['id'] .'_text' ) . '" class="of-input of-mini" name="' . esc_attr( $option_name . '[' . $value['id']. '][text]') . '" type="text" value="' . esc_attr( $prefix_text['text'] ) . '" />';
			break;
		}
